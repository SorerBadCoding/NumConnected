...
class ChatView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        conversation = ChatConversation.objects.filter(user=self.request.user).order_by("-updated_at").first()
        if conversation is None:
            conversation = ChatConversation.objects.create(user=self.request.user)
...
class NewConversationView(LoginRequiredMixin, View):
    def post(self, request):
        ChatConversation.objects.create(user=request.user)
